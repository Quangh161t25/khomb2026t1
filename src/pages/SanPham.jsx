...
export default function SanPhamPage() {
  ...
  // Pagination
  const totalPages = Math.max(1, Math.ceil(filteredProducts.length / SP_PER_PAGE));
  const paginatedProducts = useMemo(() => {
    const start = (currentPage - 1) * SP_PER_PAGE;
    return filteredProducts.slice(start, start + SP_PER_PAGE);
        ...
        <div className="overflow-x-auto flex-1 min-h-[360px] max-h-[calc(100vh-235px)] overflow-y-auto custom-scrollbar">
          <table className="w-full text-left border-collapse text-xs">
            <thead className="bg-slate-100 text-slate-700 text-xs font-bold uppercase sticky top-0 z-10 select-none shadow-2xs">
              <tr>
                <th className="px-3 py-2.5 w-36 text-left whitespace-nowrap">Mã SKU Con</th>
                <th className="px-3 py-2.5 w-24 text-center whitespace-nowrap">Mã SP Cha</th>
                <th className="px-3 py-2.5 text-left min-w-[200px]">Tên sản phẩm</th>
                <th className="px-3 py-2.5 w-28 text-right whitespace-nowrap">Giá nhập</th>
                <th className="px-3 py-2.5 w-28 text-right whitespace-nowrap">Giá bán lẻ</th>
                <th className="px-3 py-2.5 w-28 text-right whitespace-nowrap">Giá đón gói</th>
                <th className="px-3 py-2.5 w-32 text-right whitespace-nowrap">Giá thấp nhất</th>
              </tr>
            </thead>
              ...
              {paginatedProducts.length === 0 ? (
                ...
                paginatedProducts.map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 transition-colors">
                    <td className="px-3 py-2 font-bold text-indigo-700 font-mono whitespace-nowrap w-36">{item.sku_con}</td>
                    <td className="px-3 py-2 text-center text-slate-700 font-mono font-semibold whitespace-nowrap w-24">{item.id_sp}</td>
                    <td className="px-3 py-2 text-left text-slate-900 font-medium min-w-[200px] max-w-md truncate" title={item.ten_sp}>
                      {item.ten_sp || '-'}
                    </td>
                    <td className="px-3 py-2 text-right font-mono tabular-nums font-medium text-slate-700 whitespace-nowrap w-28">
          ...
          <div className="flex items-center gap-2 sm:gap-3 text-xs shrink-0">
            ...
            <div className="text-slate-600 font-medium whitespace-nowrap text-[11px] sm:text-xs">
              ...
              <strong className="text-slate-900 font-bold">
                {filteredProducts.length > 0 ? (currentPage - 1) * SP_PER_PAGE + 1 : 0} -{' '}
                {Math.min(currentPage * SP_PER_PAGE, filteredProducts.length)}
              </strong>{' '}
              trong tổng số{' '}
              <strong className="text-slate-900 font-bold">
